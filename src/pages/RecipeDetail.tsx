...
const RecipeDetail = () => {
  ...
  useEffect(() => {
    const loadRecipeData = async () => {
      setLoading(true);
      const data = await storage.getRecipesAsync();
  ...
  const handleLike = async () => {
    ...
    if (liked) {
      await appScriptApi.adjustRecipeLikes(recipe.id, -1);
  ...
  const handleDownloadPDF = () => {
    ...
    recipe.ingredients.forEach((ingredient, index) => {
      doc.text(`- ${ingredient}`, 25, 70 + index * 7);
    });
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: recipe?.title || 'Recipe', url }).catch(() => {});
      return;
    }
  ...
  if (loading) {
    return <div className="min-h-screen bg-white flex items-center justify-center font-bold text-gray-300">Loading...</div>;
  }

  if (!recipe) {
    return <div className="p-40 text-center">Recipe not found.</div>;
  }
      ...
      <div className="md:hidden">
        <div className="relative h-[50vh] w-full bg-gray-200">
          ...
          <div className="absolute top-0 left-0 right-0 p-6 pt-8 flex justify-between items-start bg-linear-to-b from-black/50 to-transparent">
            ...
            <button onClick={handleDownloadPDF} className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/40 transition-colors">
              <Download size={18} />
            </button>
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto bg-[#F4ECF4] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 min-h-[50vh]">
            <div className="mb-4">
              ...
              <div className="text-gray-400 text-sm font-medium">
                Prep: {recipe.prepTime || '—'} <span className="mx-1">|</span> Cook: {recipe.cookTime || '—'}
              </div>
            </div>
              ...
              {activeTab === 'ingredients' ? (
                <ul className="flex flex-col">
                  {recipe.ingredients.map((ing, index) => (
                    <li key={index} className={`flex items-start gap-3 py-3 ${index !== recipe.ingredients.length - 1 ? 'border-b border-[#DBCADB]' : ''}`}>
                ...
                <div className="space-y-4">
                  {(recipe.steps && recipe.steps.length > 0 ? recipe.steps : ['Enjoy your perfect creation!']).map((step, index) => (
                    <div key={index} className={`flex gap-4 py-3 ${index !== (recipe.steps?.length || 1) - 1 ? 'border-b border-[#DBCADB]' : ''}`}>
      ...
      <div className="hidden md:block bg-[#FDF7F8] min-h-screen pb-32">
        <div className="relative h-[40vh] w-full bg-gray-200 overflow-hidden">
          ...
          <div className="absolute top-0 left-0 right-0 p-8 flex justify-between items-start z-20 max-w-6xl mx-auto">
            ...
            <button onClick={handleDownloadPDF} className="w-10 h-10 bg-white/50 backdrop-blur-md rounded-full flex items-center justify-center text-gray-800 hover:bg-white transition-colors shadow-sm">
              <Download size={18} />
            </button>
          </div>
        </div>
            ...
            <div className="col-span-8 bg-[#F4ECF4] rounded-[2.5rem] shadow-xl p-10 mt-12 border border-white/50">
              <div className="mb-6">
                ...
                <div className="text-gray-500 text-base font-medium">
                  Prep: {recipe.prepTime || '—'} <span className="mx-2">|</span> Cook: {recipe.cookTime || '—'}
                </div>
              </div>
              ...
              <div className="bg-[#EBE0EC] rounded-[2.5rem] p-8 shadow-inner">
                ...
                {activeTab === 'ingredients' ? (
                  <ul className="flex flex-col">
                    {recipe.ingredients.map((ing, index) => (
                      <li key={index} className={`flex items-start gap-4 py-4 ${index !== recipe.ingredients.length - 1 ? 'border-b border-[#DBCADB]' : ''}`}>
                  ...
                  <div className="space-y-6">
                    {(recipe.steps && recipe.steps.length > 0 ? recipe.steps : ['Enjoy your perfect creation!']).map((step, index) => (
                      <div key={index} className={`flex gap-5 py-4 ${index !== (recipe.steps?.length || 1) - 1 ? 'border-b border-[#DBCADB]' : ''}`}>
