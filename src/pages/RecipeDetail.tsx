...
const RecipeDetail = () => {
  ...
  const handleLike = () => {
    ...
    if (liked) {
      ...
      localStorage.setItem('kaju_likes_tracking', JSON.stringify(localLikes.filter((id: string) => id !== slug)));
    } else {
    ...
    <div className="min-h-screen bg-white pb-0 md:pb-20">
      ...
      <div className="md:hidden">
        ...
        <div className="relative h-[50vh] w-full bg-gray-200">
            ...
            />
            {/* Top Bar Overlay */}
            <div className="absolute top-0 left-0 right-0 p-6 pt-8 flex justify-between items-start bg-gradient-to-b from-black/50 to-transparent">
               <button 
                  onClick={() => navigate(-1)}
        ...
        <div className="relative z-10 px-0 -mt-8">
          <div className="max-w-3xl mx-auto bg-[#F4ECF4] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 min-h-[50vh]">
            ...
            <div className="flex gap-3 mb-8">
                <button 
                   onClick={handleLike} 
                   className={`flex-1 py-3.5 rounded-[1.5rem] border text-sm font-semibold flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 ${liked ? 'bg-[#F4ECF4] border-[#E93C70]/30 text-[#E93C70]' : 'bg-[#EBE0EC] border-[#E0D4E0] text-gray-700'}`}
                >
                ...
                <button 
                   onClick={handleShare} 
                   className="flex-1 py-3.5 rounded-[1.5rem] bg-[#EBE0EC] border border-[#E0D4E0] text-gray-700 text-sm font-semibold flex items-center justify-center gap-2 shadow-sm transition-transform active:scale-95"
                >
            ...
            <div className="bg-[#EBE0EC] rounded-[2rem] p-6 shadow-inner">
      ...
      <div className="hidden md:block bg-[#FDF7F8] min-h-screen pb-32">
         ...
         <div className="max-w-6xl mx-auto px-6 relative z-20 -mt-32">
           <div className="grid grid-cols-12 gap-12 items-start">
             ...
             {/* Left Side Image (~30%) */}
             <div className="col-span-4 rounded-[2rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.2)] bg-white p-2">
                 <img src={recipe.imageUrl} alt={recipe.title} className="w-full aspect-[3/4] object-cover rounded-[1.5rem]" referrerPolicy="no-referrer" />
             </div>
             ...
             <div className="col-span-8 bg-[#F4ECF4] rounded-[2.5rem] shadow-xl p-10 mt-12 border border-white/50">
                ...
                <div className="flex gap-4 mb-10 max-w-md">
                    <button 
                       onClick={handleLike} 
                       className={`flex-1 py-4 rounded-[1.5rem] border text-sm font-semibold flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 ${liked ? 'bg-[#F4ECF4] border-[#E93C70]/30 text-[#E93C70]' : 'bg-[#EBE0EC] border-[#E0D4E0] text-gray-700'}`}
                    >
                    ...
                    <button 
                       onClick={handleShare} 
                       className="flex-1 py-4 rounded-[1.5rem] bg-[#EBE0EC] border border-[#E0D4E0] text-gray-700 text-sm font-semibold flex items-center justify-center gap-2 shadow-sm transition-transform active:scale-95"
                    >
